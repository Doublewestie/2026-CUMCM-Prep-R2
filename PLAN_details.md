...
| 2026-08-09 | v2.5 | Q3 M4 全量（sum_10）：DR 储能状态机逆向（T7 结案）→ 模型演进 M0/M1/M2/M3_final → 互斥物理上界 M0x（传送带假象）→ 交叉实验矩阵（斜坡/规则逐时/结算段/主时段指标）→ MPC/Sobol；三关清算定案"爬坡恶化=建模失误产物"；M3_final 主口径（时段状态机+结算段禁充+终态严格+生成器量级斜坡）；主时段指标口径；175 测试 |
