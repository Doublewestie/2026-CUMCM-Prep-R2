...
### 5.3 类选验证门（硬规则，非人肉判断）
对每个预测对象：
1. 模型池训练（时间 CV：0-2351 训练 / 2352-2375 调参，严格防泄露）
2. 评估矩阵：对象×模型 → MAPE/RMSE/区间覆盖率/区间宽度
3. 验证门：复杂模型须显著优于统计基线（MAPE 相对降幅≥5%，或覆盖率从<80% 校准到≥90%）才入选
...
F       82  79  76  25  18   5
```
功率映射：AT 0.16 / BI 0.10 / RT 0.08 MW/GPU；充放效率 0.93/0.92；MinSOC=10% 容量。
